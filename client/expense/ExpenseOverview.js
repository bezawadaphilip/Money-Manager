...
import {currentMonthPreview, expenseByCategory,currentIncome} from './../expense/api-expense.js'
import MoneyDetails from './display.js'
...
export default function Home(){
  ...
  const [income, setIncome] = useState({month:0})
  ...
  useEffect(() => {
    const abortController = new AbortController()
    const signal = abortController.signal
    currentIncome({t: jwt.token}, signal).then((data) => {
      if (data.error) {
        setRedirectToSignin(true)
      } else {
        setIncome(data)
  ...
  const indicateExpense = (values) => {
    ...
    return (
        <Card className={classes.card}>
            ...
            <MoneyDetails balanceAmount={(expensePreview.month && income.month) ? income.month.totalSpent -expensePreview.month.totalSpent : '0'}  incomeAmount={income.month ? income.month.totalSpent : '0'} expensesAmount="0" ></MoneyDetails>
